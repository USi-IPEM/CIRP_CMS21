...
The quality control of production processes and products is one fundamental part of the general quality management. It contains the measures and activities needed to fulfill specific quality requirements \cite{illes2017new}.
...
The SPC is normally used to monitor the production process using recorded data values for the quality characteristics, as well as to indicate any significant changes
in the quality characteristics in the production process \cite{selvamuthu2018introduction}. This method can also take place before the actual manufacturing process in form of an "off-line SPC". During the designing phase of a process or product,
off-line SPC procedures are aiming to select and choose controllable product and process parameters to increase the quality of the outcoming product \cite{mitra2016fundamentals}.
\AP{@Lennart, last sentence is still quite confusing to me}
...
\AP{100\% method is not introduced}
The contrary method to any statistical method, is the hundred percent control, where every product is controlled and sorted by being an ok or non-ok part.
...
an independent auditor, who based on sampling or checking records limits the finding on reporting incorrect processes procedures or products documentation. In a further step, the findings would be later corrected by other than the auditor \cite{fox1993quality}.
...
Statistical methods are faster than hundred percent controls, but have the same problem like the SPC: Because they are based on assumptions, this methods are prone to errors,
...
attributes are not measurable and can only take the value go or no-go \cite{mitra2016fundamentals}. 
...
According to Delen and Damirkan, predictive analytics (PA) consist on unravel the inherent relationships, if any, between data input and output by using data and mathematical techniques \cite{delen2013data}.

PA offers a wide range of applications, and it can be implemented as long as there is enough data available. By considering industrial and production environment, PA would be a great asset due to the high flexibility and high amound of data available coming from processes, single machines or cells, products, and others. Some common industrial applications of PA at the industry are focused products (design and optimization), machine (predictive maintenance) and production (scheduling) \cite{krauss2019machine}.
...
Based on an explorative literature review on this matter, the short outcome is that the findings that combine PA and QC are very scarce in the literature. This could be in great part, due to computer power limitations in early research and the recent increase of ML applications on this area  \cite{aumi2012model, ritter1992neue}. Related work, e.g from Krauß focused on automated machine learning for predictive quality in production \cite{krauss2020automated} and a product quality prediction in a process chain \cite{krauss2019machine}. 
...
\AP{Last paragraph goes in intro}
...
quality prediction. 
\subsection{Multilinear perceptrons}
These feedforward neural networks typically combine multiple layers
and activation functions. A layer contains a large weight matrix and
...
Due to their enormous popularity, we utilize ReLUs here as well.
...
Support Vector Machines offer a convex alternative to MLPs \cite{Suykens2002least}.
...
We would therefore expect any naive classifier to produce at least 64\% accuracy, which we have to beat. In Table~\ref{tab:class_comp}, we observe that this is indeed the case for all three approaches evaluated here. We found the random forest performed best followed by the multilinear
...
- Interpretation of data: preprocessing
   ...
   - Missing data tags: modifying PLC and robot code to have tags -> key point
        - Without label we cannot use supervised ML methods
- Dataset size
} \\
