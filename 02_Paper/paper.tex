...
With SPC, large batches can be checked, but it is only possible, if the variance of the process parameters and product quality is small enough,
...
Statistical methods such as acceptance sampling or quality control charts represent a compromise between zero and hundert percent control \cite{selvamuthu2018introduction, kurniati2015quality}.
In the case of a hundred percent control, this is associated with higher costs and higher requirement of time compared to a quality control based on a machine learning model.
Statistical methods are faster but have the same problem like the SPC: This methods are prone to errors, if the variance of the product quality is to high.
