...
Over the years, diverse methods to ensure the quality of products have been developed.
Those methods can be grouped on process-oriented or product-oriented \cite{mitra2016fundamentals}.
Among the most used process-oriented methods, we have auditing, Total Quality Control (TCQ) \cite{illes2017new}, and Poka-Yoke Techniques \cite{Robinson2011UsingPT}.
For the product-oriented methods, we have the 100\% revision of each produced product \cite{Hinckley1997QC}, and statistical methods like the Statistical Process Control (SPC) \cite{selvamuthu2018introduction}.
...
The aim of this paper is to implement a functional predictive model in quality assurance for an assembly process in a controlled environment. Within the scope of the implementation, we compare three different ML models accuracy, as well as their plausibility at industrial applications. 
As result, we expect to to decrease the gap in literature on this research area, as well as to describe the based of our experiences for future works by describing the lesson learned during this implementation.
In our work, we applied a case-based research approach \cite{yin2017case} 
% to answer the research question \AP{what research question?}.
...
Our paper is structured as follow. Section.~\ref{sec:related_works} covers the methods used in quality check in production, as well as the bases of predictive analytics in production. Section.~\ref{sec:democell} describe the demonstration cell used on this project. It answer the question of where to test predictive analytics in a controlled environment and why our system is a valid platform for this implementation. Section.~\ref{sec:ML} explains the three machine learning methods implemented on this work: Multilayer perceptrons, Support Vector Machines, Decision Tree. Section.~\ref{sec:experiment} describe the experiment process from the data acquisition and preprocessing, to the classifier optimization and testing. Finally Section.~\ref{sec:conclusion} express the remarks and discoveries achieved after the finalization of this project phase, as well as presents a list of lessons learned during the implementation of the project, which might be helpful on further implementations.


\section{Related Works} \label{sec:related_works}% 1 Page

\subsection{Quality control in production} \label{sec:qcp} % Basic books (find summarize) basic concepts 
...
% % While the SPC mostly optimizes the capability of the process, for example, the acceptance plan prevents the nonconforming product to be passed and then delivered to the next process or the customer \cite{kurniati2015quality}.
...
In this paper, we focused on the product-oriented methods at the product level.
...
% With that in mind, predictive quality control is a promising solution.
